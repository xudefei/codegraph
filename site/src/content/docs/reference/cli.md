...
`codegraph install` wires the MCP server into one or more agents. Useful flags: `--target <ids|auto|all|none>`, `--location <global|local>`, `--yes`, `--init`, `--no-permissions`, `--print-config <id>`, `--refresh`, and `--offline`.

`--offline` is for **air-gapped / self-contained installs**: run it from inside a release bundle (`release/codegraph-<target>/`) and each agent's MCP config is pointed at the bundle's absolute launcher path instead of a PATH-based `codegraph` command, and the networked `npm install -g` step is skipped. It's auto-detected when running from a bundle, so the plain `codegraph install` works offline too. Re-run install if you relocate the bundle.
